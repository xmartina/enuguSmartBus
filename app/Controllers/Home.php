...
class Home extends BaseController
{
    ...
    public function admin()
    {
        ...
        $data['dashboardCards'] = $this->buildDashboardCards($todaytrip, $totalBooking, $totalMoney, $totalPassanger);
        $data['paymentGatewayStatus'] = $this->getPaymentGatewayStatus();
        $data['recentTransactions'] = $this->getRecentTransactions();
        $data['paymentAnalytics'] = $this->getPaymentAnalytics();
        $data['refundAnalytics'] = $this->getRefundAnalytics();
        $data['disputeAnalytics'] = $this->getDisputeAnalytics();
        $data['currencySymbol'] = $this->session->get('currency_symbol') ?? '₦';

        $data['module'] =    lang("Localize.dashboard");
        $data['title']  =    lang("Localize.dashboard");

        $data['read_dashboard'] = $this->rolePermission->read("dashboard");
        $data['dashboard_denied_text'] = lang("Localize.dashboard_permission_denied");
    ...
    public function login()
    {
        ...
        $background_contents = file_get_contents(ROOTPATH . '/public/' . $settings->adminbackground);
        $background_image = base64_encode($background_contents);
    ...
    protected function buildDashboardCards($todayTrip, $todayBooking, $totalMoney, $totalPassenger)
    {
        return [
            'registeredPassengers' => $this->countPassengers(),
            'tripsToday'          => (int) $todayTrip,
            'amountPaidToday'     => (float) $totalMoney,
            'walletDeposits'      => $this->sumWalletBalances(),
            'ticketBookingsToday' => (int) $todayBooking,
            'totalBookingAmount'  => $this->sumAllTicketRevenue(),
            'refundRequests'      => $this->countRefundRequests(),
            'staffCount'          => $this->countStaff(),
            'totalPassengers'     => (int) $totalPassenger,
        ];
    }

    protected function countPassengers()
    {
        try {
    ...
    protected function sumWalletBalances()
    {
        try {
            $row = $this->db->table('wallets')->selectSum('balance')->get()->getRow();
    ...
    protected function sumAllTicketRevenue()
    {
        try {
            $row = $this->db->table('tickets')
                ...
                ->get()
                ->getRow();
            return $row ? (float) $row->paidamount : 0.0;
        } catch (\Throwable $th) {
            return 0.0;
        }
    }

    protected function countRefundRequests()
    {
        try {
            return (int) $this->db->table('tickets')->where('refund', '1')->countAllResults();
        } catch (\Throwable $th) {
            return 0;
        }
    }

    protected function countStaff()
    {
        try {
            return (int) $this->db->table('employees')->where('deleted_at', null)->countAllResults();
        } catch (\Throwable $th) {
            return 0;
        }
    }

    protected function getPaymentGatewayStatus()
    {
        return [
            'status'      => 'Active',
            'checked_at'  => date('Y-m-d H:i:s'),
            'description' => 'Gateway heartbeat verified on staging.',
        ];
    }
    ...
    protected function getPaymentAnalytics()
    {
        try {
            $totalValueRow = $this->db->table('accounts')
                ->selectSum('amount')
                ->where('type', 'income')
                ->get()
                ->getRow();
            $totalValue = $totalValueRow ? (float) $totalValueRow->amount : 0.0;

            $count = (int) $this->db->table('accounts')
                ->where('type', 'income')
                ->countAllResults();
            ...
            return [
                ...
                'averageTransaction' => $count > 0 ? $totalValue / $count : 0,
            ];
        } catch (\Throwable $th) {
            return [
                'totalProcessed'     => 0,
    ...
    protected function getRefundAnalytics()
    {
        $total = $this->countRefundRequests();
        try {
            $approved = (int) $this->db->table('tickets')
                ->where('refund', '1')
                ->where('payment_status', 'paid')
                ->countAllResults();
        } catch (\Throwable $th) {
    ...
    protected function getDisputeAnalytics()
    {
        try {
            $open = (int) $this->db->table('tickets')->where('cancel_status', '1')->countAllResults();
            $resolved = (int) $this->db->table('tickets')
