...
- **Authentication:** Complete user authentication flow with email/password and biometric readiness. Multi-step registration with profile image upload. Onboarding screen appears only on first app launch using SharedPreferences (`hasCompletedOnboarding` flag).
- **Navigation:** Custom scroll-responsive floating bottom navigation bar with 4 tabs (Book Trip, My Tickets, Luggage, Profile) with state reset on return to allow repeated access. Professional side drawer with unique menu items (Home, My Tickets, Settings, Help & Support, Logout) that don't duplicate profile screen content. Custom logo displayed in sidebar header.
- **Home Screen:** Enhanced trip search card with departure/destination inputs, date picker, passenger counter, popular routes section, and agent/loyalty promotion card. Scroll-to-top functionality when tapping Home nav while already on home.
...
- **Payment Success Screen:** Payment confirmation screen with success icon and BACK TO HOME button that clears navigation stack and routes to home screen.
