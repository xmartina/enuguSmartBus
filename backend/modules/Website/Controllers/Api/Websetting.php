...
class Websetting extends BaseController
{
	...
	public function index()
	{
		...
		else
		{
			...
			$result['pay_later'] = $websetting->pay_later == '1' ? true : false;
			$result['luggage_service'] = $websetting->luggage_service == '1' ? true : false;
			$result['chat_tawk'] = $websetting->chat_tawk == '1' ? true : false;
			if($websetting->chat_tawk == '1'){
				$tawk_info	= $this->liveChatModel->where('name', 'tawk')->where('status', 1)->first();
				if(!empty($tawk_info)){
                    $result['property_id'] = $tawk_info->property_id;
					$result['widget_id'] = $tawk_info->widget_id;
                }else{
					$result['property_id'] = '';
					$result['widget_id'] = '';
				}
