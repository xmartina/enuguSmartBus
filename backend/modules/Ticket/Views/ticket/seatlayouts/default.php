<div style="border-width: 8px; border-style: solid; border-color: rgb(119, 119, 119) rgb(211 213 215) rgb(195 195 195); max-width: 320px; width: 100%; height: 100%; margin: 0px auto; background-color: #fff;">
    ...
    <div style="border-bottom: 1px solid rgb(217 217 217); padding: 5px 0px">
        ...
        <?php foreach ($seatRows as $singleRow) : ?>
            <ul style="display: grid; grid-template-columns: repeat(<?php echo count($singleRow); ?>, 1fr); align-items: flex-start; -webkit-box-pack: center; justify-content: center; list-style: none; margin: 0px; padding: 5px 0px">

                <?php foreach ($singleRow as $singleSeat) : ?>

                    <?php 
                    if (is_array($singleSeat)) :
                        $seatLabel = $singleSeat['seatName'];
                        $seatLabelThumb = $singleSeat['isBooked'] ? 'booked' : 'available';    
                    ?>
                        <li style="cursor: pointer; margin: auto; position: relative; user-select: none;">
                            <div onclick="<?php echo "seacClick(this, $subTripId, '$seatLabelThumb');"; ?>" data-seatnumber="<?php echo $seatLabel; ?>">
                                ...
                                <p style="padding: 0px; margin: 0px; position: absolute; left: 50%; top: 40%; transform: translate(-50%, -50%); transition: all 0.4s ease 0s; font-size: 10px; color: rgb(68, 68, 68); font-weight: 900"><?php echo $seatLabel; ?></p>
                            </div>
                        </li>
                    <?php else: ?>
