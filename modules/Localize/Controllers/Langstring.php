...
class Langstring extends BaseController
{
    ...
    public function bulkDownloadSample()
    {
        ...
        $lngStringValues = $this->langstringvalueModel
            ...
            ->select(new RawSql('GROUP_CONCAT(IFNULL(lngstngvalues.value, "") SEPARATOR "_-_") AS `value`'))
        ...
        // create csv template with escaping for commas and quotes
        $fileHeader = implode(',', array_map(fn($header) => "\"$header\"", array_keys(current($stringValueChart))));
        $fileBody = implode(PHP_EOL, array_map(fn ($row) => implode(',', array_map(function ($value) {
            // Escape double quotes and wrap value in quotes if it contains a comma or quote
            $escapedValue = str_replace('"', '""', $value);
            return strpos($value, ',') !== false || strpos($value, '"') !== false ? "\"$escapedValue\"" : $escapedValue;
        }, $row)), $stringValueChart));
