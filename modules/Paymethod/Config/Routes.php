...
$routes->group('modules/api/v1/paymethods', ["filter" => "cors", "namespace" => "\Modules\Paymethod\Controllers\Api"], function ($routes) {
     ...
     // Paystack group
    $routes->group('paystack', function ($routes) {
        $routes->post('initialize', 'Paystack::initialize');          // POST initialize
        $routes->get('verify/(:segment)', 'Paystack::verify/$1');    // GET verify/{reference}
        $routes->get('callback', 'Paystack::callback');              // GET callback?reference=...
        $routes->post('webhook', 'Paystack::webhook');               // POST webhook
    });
    ...
    $routes->get('flutterwave', 'Paymentgateway::flutterWave');
     //$routes->get('paystack-callback', 'Paymentgateway::paystackCallback');
    ...
    $routes->group('stripe', function ($routes) {
        $routes->post('', 'Paymentgateway::stripePayment');
        $routes->get('stripe-callback', 'Paymentgateway::stripeCallback', ['as' => 'stripe-payment-callback']);
    });
});
